 ...
 * Upload Controller
 */
class LightchecklistControllerEditprofile extends JControllerForm
{
		public function uploadFile()
		{
			...
			if (empty($avatar_filename)) {
				$message = JText::_('COM_LIGHTCHECKLIST_PLEASE_SELECT_AN_IMAGE');
			}
					
					
			if (JFile::exists($base_Dir.$userid.'/'.$avatar_filename)) {
				$message = JText::_('COM_LIGHTCHECKLIST_IMAGE').$avatar_filename.JText::_('COM_LIGHTCHECKLIST_ALREADY_EXISTS');
			}
			...
			if ((strcasecmp(substr($avatar_filename,-4),".gif")) && (strcasecmp(substr($avatar_filename,-4),".jpg")) && (strcasecmp(substr($avatar_filename,-4),".png")) && (strcasecmp(substr($avatar_filename,-4),".bmp")) ) {
				$message = JText::_('COM_LIGHTCHECKLIST_ACCEPTED_FILES');
			}
			...
			if (!JFile::move($avatar_file_tmpname, $base_Dir.$userid.'/'.$avatar_filename) || !JPath::setPermissions($base_Dir.$userid.'/'.$avatar_filename)) {
				$message = JText::_('COM_LIGHTCHECKLIST_UPLOAD_OF').$avatar_filename.JText::_('COM_LIGHTCHECKLIST_FAILED');
				$error = true;
			} else {
				$message = JText::_('COM_LIGHTCHECKLIST_UPLOAD_OF')." ".$avatar_filename.JText::_('COM_LIGHTCHECKLIST_SUCCESSFUL');
			}

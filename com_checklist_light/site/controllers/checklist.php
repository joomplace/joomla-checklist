...
class LightchecklistControllerChecklist extends JControllerAdmin
{
        public function addgroup(){
			...
			<h2 class="checklist-section-header">'.$title.'<span class="chk-add-item"><img style="cursor:pointer;" src="'.JURI::root().'components/com_lightchecklist/assets/images/minus.png" class="chk-ajax-remove-group" />&nbsp;<img style="cursor:pointer;" src="'.JURI::root().'components/com_lightchecklist/assets/images/list_add.png" class="chk-open-item-form" /></span></h2>
		...
		public function additem(){
			...
			require_once(JPATH_SITE.'/components/com_lightchecklist/assets/markdown/Parsedown.php');
			...
			if($template == 'default'){
				...
				$html = '<li '.$class.' itemid="'.$lastid.'"><input type="checkbox" tabindex="'.($max_ordering+1).'" id="'.$input_id.'" class="chk-checkbox"><label for="'.$input_id.'">'.$title.'<a id="details-'.$input_id.'" class="checklist-info-icon" data-parent="#chk-main" data-toggle="collapse"><span class="glyphicon glyphicon-info-sign"></span></a><img src="'.JURI::root().'components/com_lightchecklist/assets/images/trash_recyclebin_empty_closed_w.png" class="chk-tools" onclick="Checklist.ajaxRemoveItem(this, event);"/><img src="'.JURI::root().'components/com_lightchecklist/assets/images/pencil2.png" class="chk-tools pencil" onclick="Checklist.editItem(this, event);"/></label><ul class="panel-collapse collapse"><li>'.$tips.'</li></ul></li>';
			} else {
				$class = ($optional) ? 'class="optional li-handle"' : 'class="li-handle"';
				$html = '<li '.$class.' itemid="'.$lastid.'"><input type="checkbox" tabindex="'.($max_ordering+1).'" id="'.$input_id.'"><label for="'.$input_id.'">'.$title.'<img src="'.JURI::root().'components/com_lightchecklist/assets/images/trash_recyclebin_empty_closed_w.png" class="chk-tools" onclick="Checklist.ajaxRemoveItem(this, event);"/><img src="'.JURI::root().'components/com_lightchecklist/assets/images/pencil2.png" class="chk-tools pencil" onclick="Checklist.editItem(this, event);"/></label><em class="" id="details-'.$input_id.'"></em><ul style="max-height: 0px;" class="checklist-section-details"><li>'.$tips.'</li></ul></li>';
			}
			...
			$html = '<form class="form-horizontal chk-edititem-form" id="chk-edititem-form-'.$itemid.'" style="">
					...
					<div class="form-group">
						<div class="col-sm-offset-2 col-sm-12">
							...
							<br/><img id="ajax_loader_'.$itemid.'" style="display: none;" src="'.JURI::root().'components/com_lightchecklist/assets/images/ajax-loader.gif">
						</div>
					</div>
				</form>';
		...
		public function switchEditMode()
		{
			...
			$sefUrl = JRoute::_('index.php?option=com_lightchecklist&view=checklist&id='.$checklist_id);
		...
		public function sendRequest()
		{
			...
			if($config->useNotification && $config->emailsNotification!=''){
				...
				if (count($emails)){
					...
					$request_url = JURI::root().'administrator/index.php?option=com_lightchecklist&view=requests&checklist_id='.$checklist_id;
					$userName = $user->name;

					$mailer->addRecipient($emails);
					$body   = sprintf(JText::_('COM_LIGHTCHECKLIST_REQUEST_EMAIL_BODY'), $userName, $request_url);
					$mailer->isHTML(true);
					$mailer->setSubject(JText::_('COM_LIGHTCHECKLIST_REQUEST_EMAIL_SUBJECT'));
					$mailer->setBody($body);

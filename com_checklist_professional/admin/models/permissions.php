...
class ChecklistModelPermissions extends JModelLegacy
{
	//----------------------------------------------------------------------------------------------------
	public static function SaveComponentAsset($rules)
	{
		...
		self::UpdateExistingAsset('com_checklist', $rules);
	}
	//----------------------------------------------------------------------------------------------------
	public static function SavePublicationAsset($listId, $rules)
	{
		$publicationAssetName = 'com_checklist.list.'.$listId;
	...
	public static function ResetAllAssets()
	{
		...
		$query = "SELECT * FROM `#__assets`" .
			" WHERE `name` REGEXP '" . "com_checklist.list." . "'";
			" ORDER BY `id`";
		$db->setQuery($query);
		...
		if (count($listsWithoutAsset) > 0)
		{
			...
			{
				$listAssetName = 'com_checklist.list.'.$list->id;
				$title = $list->title;
				$rules = "";
	...
	private static function GetComponentRootAssetId()
	{
		...
		$query = "SELECT `id` FROM `#__assets`" .
			" WHERE `name` = " . $db->quote('com_checklist');
		$db->setQuery($query);
		$componentRootAssetId = $db->loadResult();
		
		if (!isset($componentRootAssetId)) throw new Exception(JText::_('COM_CHECKLIST_BE_NO_COMPONENT_ASSET'));

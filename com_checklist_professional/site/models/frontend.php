...
class ChecklistModelFrontend extends JModelList
{
    ...
    public function getChecklists($isTotal = 0)
    {
        ...
        if ($this->config->rating_option) {
            $options[] = JHTML::_('select.option', 'rating', JText::_('COM_CHECKLIST_RATING_FILTER'));
        }
    ...
    public function sortByRating($available_checklists, $filter_order_Dir)
    {
        $ratings = array();
        if (count($available_checklists)) {
            foreach ($available_checklists as $checklist) {
                $ratings[$checklist->id] = $checklist->average_rating;
            }
        }
        ...
        $tmp_checklists = array();
        if (count($ratings)) {
            foreach ($ratings as $checklist_id => $rating) {
                foreach ($available_checklists as $checklist) {
                    if ($checklist->id == $checklist_id) {
                        $tmp_checklists[] = $checklist;
                    }
                }
            }
        }

        return $tmp_checklists;
    }

 ...
 * Install Controller
 */
class ChecklistControllerInstall extends JControllerAdmin
{
	...
	*/
	public function plugins(){
		$allowContinue = true;
		include_once(JPATH_ROOT . '/administrator/components/com_checklist/installer/plugins.html');
		exit;
	}
	...
	public function install_plugins(){
		...
		$plugins		= array();
		$plg_names 		= array();
		$source			= JPATH_ROOT . '/components/com_checklist/checklist_plugins.zip';
		$destination	= JPATH_ROOT . '/components/com_checklist/checklist_plugins/';
		...
		{
			$plugins[] = JPATH_ROOT . '/components/com_checklist/checklist_plugins/plg_checklistuser.zip';
			
			if(count($_REQUEST['jform'])){
				foreach($_REQUEST['jform'] as $plg_name => $enable){
					if($enable){
						$plugins[]     = JPATH_ROOT . '/components/com_checklist/checklist_plugins/plg_'.$plg_name.'.zip';
						$plg_names[] = $plg_name;
					}
				}
			}
		...
		if(empty($plugins)){
			...
			. "parent.jQuery('#jq_install_btn').attr('onclick', 'location.href=\'index.php?option=com_checklist&task=install.modules\'');"
			. "</script>";
			@flush();
			@ob_end_flush();
			...
			JFile::delete(JPATH_ROOT . '/components/com_checklist/checklist_plugins.zip');
			die;
		}
		...
		if(count($_REQUEST['jform'])){
			...
			$this->_enablePlugin('checklistuser');
		}
	...
	*/
	public function modules(){
		$allowContinue = true;
		include_once(JPATH_ROOT . '/administrator/components/com_checklist/installer/modules.html');
		exit;
	}
	...
	function install_modules(){
		...
		$modules		= array();
		$source			= JPATH_ROOT . '/components/com_checklist/checklist_modules.zip';
		$destination	= JPATH_ROOT . '/components/com_checklist/checklist_modules/';
		...
		if(JArchive::extract($source, $destination))
		{
			if(count($_REQUEST['jform'])){
				foreach($_REQUEST['jform'] as $mod_name => $enable){
					if($enable){
						$modules[]     = JPATH_ROOT . '/components/com_checklist/checklist_modules/'.$mod_name.'.zip';
					}
				}
			}
		}
		...
		if(empty($modules)){
			...
			. "parent.jQuery('#jq_install_btn').attr('onclick', 'location.href=\'index.php?option=com_checklist&task=install.done\'');"
			. "</script>";
			@flush();
			@ob_end_flush();
			...
			//remove temp folder
			JFolder::delete($destination);
			JFile::delete(JPATH_ROOT . '/components/com_checklist/checklist_modules.zip');
			die;
		}
		$app = JFactory::getApplication();
	...
	public function done(){
		...
		if(file_exists(JPATH_ROOT . '/components/com_checklist/checklist_modules.zip')){
			//remove temp zip archive
			JFile::delete(JPATH_ROOT . '/components/com_checklist/checklist_modules.zip');
		}

		if(file_exists(JPATH_ROOT . '/components/com_checklist/frontend.zip')){
			//remove temp zip archive
			JFile::delete(JPATH_ROOT . '/components/com_checklist/frontend.zip');
		}
		
		$allowContinue = true;
		include_once(JPATH_ROOT . '/administrator/components/com_checklist/installer/done.html');
		exit;
	}

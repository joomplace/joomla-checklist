...
class ChecklistControllerRequests extends JControllerAdmin
{	
	...
	public function ajaxconfirm()
	{
		...
		if($checklist_id){
			...
			$subject = JText::_('COM_CHECKLIST_USER_CONFIRM_NOTIFICATION_EMAIL_SUBJECT');
	...
	public function ajaxreject(){
		...
		if($checklist_id){
			...
			//Send email notification
			$message = sprintf($message, $user->name);
			$subject = JText::_('COM_CHECKLIST_USER_REJECT_NOTIFICATION_EMAIL_SUBJECT');
			$emails = array($user->email);
